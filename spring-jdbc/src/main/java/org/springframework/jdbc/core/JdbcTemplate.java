...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	public <T> T execute(StatementCallback<T> action) throws DataAccessException {
		...
		Statement stmt = null;
		try {
			//直接使用Connection实例创建Statement对象，不同于PreparedStatement使用PreparedStatementCreator来创建
	...
	public <T> T query(final String sql, final ResultSetExtractor<T> rse) throws DataAccessException {
		...
		class QueryStatementCallback implements StatementCallback<T>, SqlProvider {
			...
			public T doInStatement(Statement stmt) throws SQLException {
				ResultSet rs = null;
				try {
					//对statement类型的sql真正的执行其实是委托了给executeQuery()
	...
	public <T> T queryForObject(String sql, Class<T> requiredType) throws DataAccessException {
		//使用SingleColumnRowMapper封装所需的类型，并返回相应的实例
	...
	/*execute()是数据库操作的核心入口，对大多数数据库操作相同的步骤统一封装，
	*而对个性化操作使用PreparedStatementCallback进行回调*/
	@Override
	@Nullable
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		try {
			...
			//应用用户设定的输入参数，主要是对FetchSize、MaxRows和QueryTimeOut进行设置
			applyStatementSettings(ps);
			//调用回调函数，主要是用于对通用方法外的个性化方法的处理
			T result = action.doInPreparedStatement(ps);
			//警告处理，若没有忽略warnings警告，则抛出SQLWarningException异常或者打印出相关信息
			handleWarnings(ps);
			return result;
		}
		catch (SQLException ex) {
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			//释放数据库连接，避免当异常转换器未来得及被初始化时出现潜在的连接池死锁
			...
			ps = null;
			//释放获取到的数据源
			/*这里考虑存在事务的情况--若当前线程中存在事务，即表明在当前线程中存在共用数据库连接--直接使用ConnectionHolder
			* 的released()进行连接数减1，并不是真正的释放连接*/
			DataSourceUtils.releaseConnection(con, getDataSource());
			con = null;
		...
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					...
					//JDBC中真正执行对数据库进行查找的核心逻辑
					rs = ps.executeQuery();
					//extractData()：将获取到的ResultSet进行封装并转换至POJO
					//rse：代表RowMapperResultSetExtractor实例对象，且在构造rse实例时又封装了自定义的rowMapper
					return rse.extractData(rs);
				}
				finally {
					JdbcUtils.closeResultSet(rs);
	...
	public <T> T query(String sql, @Nullable PreparedStatementSetter pss, ResultSetExtractor<T> rse) throws DataAccessException {
		//在这里，同update()，使用SimplePreparedStatementCreator对传入的sql语句进行封装
	...
	public <T> T query(String sql, Object[] args, int[] argTypes, ResultSetExtractor<T> rse) throws DataAccessException {
		//在这里，同update()中一样，均使用ArgumentTypePreparedStatementSetter去封装传入的参数和参数类型
	...
	protected int update(final PreparedStatementCreator psc, @Nullable final PreparedStatementSetter pss)
		...
		return updateCount(execute(psc, ps -> {
			try {
				if (pss != null) {
					//用参数值对给定的PreparedStatement-SQL语句中的占位符进行替换
					//对该函数的真正实现其实是委托了给ArgumentTypePreparedStatementSetter
					pss.setValues(ps);
				}
				//ps.executeUpdate()才是真正执行SQL的核心
	...
	public int update(String sql, Object[] args, int[] argTypes) throws DataAccessException {
		//使用ArgumentTypePreparedStatementSetter对参数类型和参数进行封装
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		/*fetchSize是指当调用re.next()时，ResultSet会一次性从服务器上取得多少行数据，
		* 这样，下次调用re.next()时可直接从内存中获取，无需通过网络进行交互*/
		int fetchSize = getFetchSize();
		if (fetchSize != -1) {
			//设置FetchSize的最主要目的是为了减少网络交互的次数
			stmt.setFetchSize(fetchSize);
		}
		int maxRows = getMaxRows();
		if (maxRows != -1) {
			//将此statement对象生成的所有ResultSet对象可包含的最大行数限制设置为给定数
	...
	protected void handleWarnings(Statement stmt) throws SQLException {
		//当设置为忽略警告时只尝试打印日志，默认是忽略警告
		if (isIgnoreWarnings()) {
			//如果日志开启的情况下打印日志
			if (logger.isDebugEnabled()) {
				/*SQLWarning提供关于数据库访问警告信息的异常，这些警告直接链接到导致报告警告的方法所在的对象。
				* 警告可以从Connection、Statement和ResultSet对象中获得。试图在已经关闭的连接上获取警告将导致抛出异常。
				* 类似的，试图在已经关闭的Statement对象或已经关闭的ResultSet上获取警告也将导致抛出异常。
				* 注意，关闭statement对象还会关闭该对象产生的ResultSet对象*/
