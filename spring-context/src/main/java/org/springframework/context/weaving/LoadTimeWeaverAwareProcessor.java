...
public class LoadTimeWeaverAwareProcessor implements BeanPostProcessor, BeanFactoryAware {
	...
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		//过滤bean的类型，只对LoadTimeWeaverAware类型的bean起作用，就目前而言，实现LoadTimeWeaverAware接口的类只用AspectJWeavingEnabler
		if (bean instanceof LoadTimeWeaverAware) {
			LoadTimeWeaver ltw = this.loadTimeWeaver;
			//若当在spring中调用AspectJWeavingEnable时，this.loadTimeWeaver尚未被初始化，则会直接调用beanFactory.getBean()获取
			//DefaultContextLoadTimeWeaver类型的bean
			if (ltw == null) {
			...
			//并将该bean设置在AspectJWeavingEnabler类型的bean的loadTimeWeaver属性中
