...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		//向Spring容器注册了一个BeanName为internalAutoProxyCreator的Bean定义
		//可以自定义也可以使用Spring提供的（根据优先级来）
		//Spring默认提供的是AspectJAwareAdvisorAutoProxyCreator，这个类是AOP的核心类
	...
	private void parseAspect(Element aspectElement, ParserContext parserContext) {
		...
		try {
			...
			//处理<aop:aspect>标签下的<aop:before>、<aop:after>、<aop:after-returning>、<aop:after-throwing method="">、<aop:around method="">
