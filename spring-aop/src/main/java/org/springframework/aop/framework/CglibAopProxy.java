...
class CglibAopProxy implements AopProxy, Serializable {
	...
	/*CGLIB对方法的拦截是通过将自定义的拦截器(实现MethodInterceptor接口)加入*/
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		//将advised属性封装在DynamicAdvisedInterceptor中，并加入到callbacks中
		//DynamicAdvisedInterceptor继承自MethodInterceptor，在加入Callback后，再次调用代理时会直接调用其重写的intercept()，
		//该方法也是真正对CGLIB方式实现的核心所在
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		/*Cglib的实现方式基本同JDK的实现方式：先构造链，然后封装该链进行串行调用
		* 不同点：JDK中直接构造ReflectiveMethodInvocation，在Cglib中使用CglibMethodInvocation(继承自ReflectiveMethodInvocation，
		* 但proceed没有重写)*/
		@Override
		@Nullable
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					//用CglibMethodInvocation封装链表，并进入链中执行处理
