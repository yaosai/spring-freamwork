...
public class ResourceEditorRegistrar implements PropertyEditorRegistrar {
	...
	/**
	 * Populate(填充) the given {@code registry} with the following resource editors:
	...
	public void registerCustomEditors(PropertyEditorRegistry registry) {
		/*该函数的主要作用是注册一系列常用类型的属性编辑器，最终是使用了
		 *registerCustomEditor(Class<?> requiredType, PropertyEditor propertyEditor)*/
		/*这里的目的：在对bean实体注入属性时，若存在特殊类型属性，那么spring则会调用对应的编辑器，将配置中定义的
		 *String类型转换为特殊类型并进行赋值*/
		/*该函数会被AbstractBeanFactory的initBeanWrapper(主要用于将BeanDefinition转换为BeanWrapper后用于对属性的填充)调用。
		 *在bean的初始化后会调用该方法进行批量的通用属性编辑器的注册，注册后，在属性填充时便可直接让spring使用这些编辑器进行属性的解析*/
