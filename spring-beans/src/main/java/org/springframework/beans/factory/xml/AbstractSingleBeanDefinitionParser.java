...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected final AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
		//使用GenericBeanDefinition来构造BeanDefinitionBuilder实例
		BeanDefinitionBuilder builder = BeanDefinitionBuilder.genericBeanDefinition();
		String parentName = getParentName(element);
		if (parentName != null) {
			builder.getRawBeanDefinition().setParentName(parentName);
		}
		//获取自定义标签对应的class对象，该方法会调用子类中重写的getBeanClass方法获取对应的class对象
		Class<?> beanClass = getBeanClass(element);
		if (beanClass != null) {
			builder.getRawBeanDefinition().setBeanClass(beanClass);
		}
		else {
			//若子类没有重写getBeanClass方法，则调用重写的getBeanClassName方法
		...
		if (containingBd != null) {
			...
			//内部bean定义必须同父类bean的scope
			//若存在父类，则使用父类的scope
			builder.setScope(containingBd.getScope());
		}
		if (parserContext.isDefaultLazyInit()) {
			// Default-lazy-init applies to custom bean definitions as well.
			//默认懒加载同样适用于用户自定义的bean定义
			//配置延迟加载
	...
	/**
	 * Determine the bean class name corresponding to（对应于） the supplied {@link Element}.
	 * @param element the {@code Element} that is being parsed
