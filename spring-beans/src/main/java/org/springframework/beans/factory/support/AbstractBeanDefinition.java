...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	@Nullable
	/* bean的作用范围，对应<bean>的scope属性 */
	private String scope = SCOPE_DEFAULT;
	/* 是否抽象，对应<bean>的abstract属性 */
	private boolean abstractFlag = false;
	/* 是否延迟加载，对应<bean>的lazy-init属性 */
	private boolean lazyInit = false;
	/* 自动注入模式，对应<bean>的autowire */
	private int autowireMode = AUTOWIRE_NO;
	/* 依赖检查，spring3.0后弃用该属性 */
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;

	/* 用来表示一个bean的实例化依靠另一个bean先实例化，对应<bean>的depend-on属性 */
	@Nullable
	private String[] dependsOn;
	/**对应<bean>的autowire-candidate属性
	 * 若autowire-candidate=false，容器在查找自动装配对象时，将不考虑该bean
	 * 即当前bean不会被考虑作为其他bean自动装配的候选者，但该bean本身可使用自动装配来注入其他bean
	 */
	private boolean autowireCandidate = true;
	/**对应<bean>primary属性
	 * 自动装配时若出现多个bean候选者，当前bean是否作为首选者
	 */
	private boolean primary = false;
	/* 对应<bean>的<qualifier>子节点 */
	private final Map<String, AutowireCandidateQualifier> qualifiers = new LinkedHashMap<>();

	@Nullable
	private Supplier<?> instanceSupplier;
	/* 是否允许访问非公开的构造器和方法，默认为true，程序设置 */
	private boolean nonPublicAccessAllowed = true;
	/**是否以一种宽松的模式解析构造函数，默认为true，程序设置
	 * 若设置为false，则在如下情况会抛出异常，因为spring无法准确定位哪个构造函数
	 * interface ITest{}
	...
	/* 用于引用其他工厂类在容器中唯一标识的bean_id，对应<bean>的factory-bean属性 */
	@Nullable
	private String factoryBeanName;
	/* 用于指定当前bean所引用的工厂类实例中的工厂方法，对应<bean>的factory-method属性 */
	@Nullable
	private String factoryMethodName;
	/* 记录构造函数的参数对应注入的值，对应<bean>的<constructor-arg>子节点 */
	@Nullable
	private ConstructorArgumentValues constructorArgumentValues;
	/* 记录bean中普通属性及对应注入的值，对应<bean>的<property>子节点 */
	@Nullable
	private MutablePropertyValues propertyValues;
	/* 方法重写的持有者，对应<bean>的<lookup-method>、<replaced-method>元素 */
	@Nullable
	private MethodOverrides methodOverrides;
	/* 用于指定初始化方法，对应<bean>的init-method属性 */
	@Nullable
	private String initMethodName;
	/* 用于指定销毁方法，对应<bean>的destory-method属性 */
	@Nullable
	...
	*/
	...
	/* bean的描述信息，对应<bean>的<description>子节点 */
	@Nullable
